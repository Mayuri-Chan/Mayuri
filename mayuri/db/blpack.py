...
class Blpack(BASE):
	...
	def __init__(self,chat_id,packname,mode,reason,duration):
...
def add_to_blpack(chat_id,packname,mode,reason,duration):
	with BLPACK_INSERTION_LOCK:
		...
		Blpack_filt = Blpack(chat_id,packname,mode,reason,duration)
...
def check_pack(chat_id, packname):
	curr = SESSION.query(Blpack).get((str(chat_id), packname))
	if curr:
		return curr
	return False
